...
def is_next_button(device_id):
    run_adb_command(f"adb -s {device_id} shell uiautomator dump /sdcard/window_dump.xml")
    run_adb_command(f"adb -s {device_id} pull /sdcard/window_dump.xml .")
    with open("window_dump.xml", "r", encoding="utf-8") as file:
        xml_content = file.read()
    return "com.whatsapp.w4b:id/registration_submit" in xml_content
...
def check_and_click_next_button(selected_device,setup_data):
    while(True):
        if is_next_button(selected_device):
...
def check_if_one_hour_came(setup_data,selected_device):
...
def automate_login(selected_device,setup_data):
    ...
    for index,phone_number in enumerate(phone_numbers,start=1):
        ...
        check_and_click_next_button(selected_device,setup_data)
        ...
        check_if_one_hour_came(setup_data,selected_device)
